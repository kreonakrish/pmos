"""REASONING memory service — distilled reasoning patterns stored in MySQL + FAISS."""
...
class ReasoningMemoryService:
    ...
    async def store(self, agent_id: int, pattern: str, context: dict[str, Any]) -> str:
        """Store a reasoning pattern with embedding. Returns memory_id."""
        span_id = new_span_id()
        memory_id = str(uuid.uuid4())

        vector = await self._embedding.embed(pattern)
        ...
        row = {
            ...
            "relevance_score": context.get("importance", 0.7),
            "access_count": 0,
            "last_accessed": None,
            "decay_factor": 1.0,
            "expires_at": None,
    ...
    async def search(self, agent_id: int, query: str, k: int = 3) -> list[dict[str, Any]]:
        ...
        faiss_hits = await self._faiss.search(agent_id, query_vec, k * 3)  # oversample
        ...
        rows = await self._mysql.execute(
            f"SELECT * FROM {TABLE} WHERE content_vector_id IN ({placeholders}) AND memory_tier = '{TIER}'",
            tuple(memory_ids),
            fetch=True,
        )
